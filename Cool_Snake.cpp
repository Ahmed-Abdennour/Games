...
int main()
{
	...
	while(1)
	{
		...
		while (!bDead)
		{
			...
			while ((chrono::system_clock::now() - t1) < ((nSnakeDirection % 2 == 1) ? 60ms : 80ms))
			{
				// Get Input, 
				bKeyRight = (0x8000 & GetAsyncKeyState((unsigned char)('\x27'))) != 0;
					if (bKeyRight && !bKeyLeft && !bKeyUp && !bKeyDown && nSnakeDirection != 3) {nSnakeDirection = 1; }
				bKeyLeft = (0x8000 & GetAsyncKeyState((unsigned char)('\x25'))) != 0;
					if (bKeyLeft && !bKeyRight && !bKeyUp && !bKeyDown && nSnakeDirection != 1) {nSnakeDirection = 3; }
				bKeyUp = (0x8000 & GetAsyncKeyState((unsigned char)('\x26'))) != 0;
					if (bKeyUp && !bKeyRight && !bKeyLeft && !bKeyDown && nSnakeDirection != 2) {nSnakeDirection = 0; }
				bKeyDown = (0x8000 & GetAsyncKeyState((unsigned char)('\x28'))) != 0;
					if (bKeyDown && !bKeyRight && !bKeyLeft && !bKeyUp && nSnakeDirection != 0) {nSnakeDirection = 2; }
			...
			wsprintf(&screen[nScreenWidth + 8], L"         S N A K E ! ! - PRETTY COOL HUH ?                          SCORE: %d", nScore * 4);
